...
namespace :spec do
  ...
  namespace :db do
    namespace :fixtures do
      ...
      task :load => :environment do
        ...
        (ENV['FIXTURES'] ? ENV['FIXTURES'].split(/,/) : Dir.glob(File.join(Rails.env, 'spec', 'fixtures', '*.{yml,csv}'))).each do |fixture_file|
  ...
  namespace :server do
    daemonized_server_pid = File.expand_path("spec_server.pid", Rails.env + "/tmp")
