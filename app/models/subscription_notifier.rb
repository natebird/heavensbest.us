class SubscriptionNotifier < ActionMailer::Base
 ...
 # Remove this 
  def welcome(user)
    setup_email(user, "Welcome to #{APP_CONFIG[:site_name]}!")
    @body = { :user => user }
  end
  ...
  def charge_receipt(subscription_payment)
    setup_email(subscription_payment.account.user, "Your #{APP_CONFIG[:site_name]} invoice")
  ...
  def setup_receipt(subscription_payment)
    setup_email(subscription_payment.subscription.account.user, "Your #{APP_CONFIG[:site_name]} invoice")
    @body = { :subscription => subscription_payment.subscription, :amount => subscription_payment.amount }
  end
  
  def charge_failure(subscription)
    setup_email(subscription.account.user, "Your #{APP_CONFIG[:site_name]} renewal failed")
    @bcc = APP_CONFIG[:from_email]
    @body = { :subscription => subscription }    
  end
  
  def plan_changed(subscription)
    setup_email(subscription.account.user, "Your #{APP_CONFIG[:site_name]} plan has been changed")
    @body = { :subscription => subscription }    
  end
